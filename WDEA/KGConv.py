...
from WDEA.layernormalization import STDLayerNorm as LayerNorm

class WassersteinAttention(nn.Module):
    def __init__(self, input_size, hidden_size, num_heads):
        super(WassersteinAttention, self).__init__()
        self.mu = nn.Linear(input_size, hidden_size)
        self.conv = nn.Linear(input_size, hidden_size)
        self.attn_mu = nn.Parameter(torch.FloatTensor(size=(num_heads, hidden_size)), requires_grad=True)
        self.attn_conv = nn.Parameter(torch.FloatTensor(size=(num_heads, hidden_size)), requires_grad=True)
        nn.init.xavier_normal_(self.mu.weight.data, gain=1.414) 
        nn.init.xavier_normal_(self.conv.weight.data, gain=1.414) 
        nn.init.xavier_normal_(self.attn_mu.data, gain=1.414) 
        nn.init.xavier_normal_(self.attn_conv.data, gain=1.414) 

    def forward(self, x):
        mean1 = self.mu(x)
        cov1 = self.conv(x)
        d = -self.WassersteinDistance(mean1, cov1, self.attn_mu, self.attn_conv)
        a = torch.softmax(d,dim=-1)
        return  a
    
    def WassersteinDistance(self,mean1, cov1, mean2, cov2):
...
class WDGCNKGlayer(nn.Module):
    def __init__(self,
                 ...
                 top_k=-1,
                 negative_slope=0.2):
        super(WDGCNKGlayer, self).__init__()
        ...
        self.watt = WassersteinAttention(input_size=in_ent_feats,hidden_size=in_ent_feats,num_heads=num_heads)
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_normal_(self.fc_ent_head.weight.data, gain=1.414)   # wh
        nn.init.xavier_normal_(self.fc_ent_tail.weight.data, gain=1.414)   # wt
        nn.init.xavier_normal_(self.fc_ent.weight.data, gain=1.414)
        nn.init.xavier_normal_(self.fc_ent_out.weight.data, gain=1.414)
        nn.init.xavier_normal_(self.fc_rel.weight.data, gain=1.414)   # rk
    ...
    def forward(self, graph: DGLGraph, ent_embed, rel_embed, edge_ids=None, drop_edge_ids=None):
        ...
        h_r = self.graph_rel_norm(rel_embed) # (671, 512) (relation number, hidden)
        rel_feat = torch.tanh(self.fc_rel(self.input_drop(h_r))).view(-1, self._num_heads, self.att_dim)
        h = self.graph_ent_norm(ent_embed)#(13053, 512) (num-ent, hidden)
        ...
        ew = self.watt(h).unsqueeze(-1)
        # rw = self.watt(h_r).unsqueeze(-1)
        er_relation = (rel_feat * self.attn_r).sum(dim=-1).unsqueeze(-1)  
        e_ids = graph.edata['e_label'].squeeze(dim=-1)
        e_ids = e_ids.type(torch.int64)
        er = er_relation[e_ids]    # 找到每个边对应的表示


        ent_feat = self.fc_ent(self.input_drop(h)).view(-1, self._num_heads, self.att_dim)#(13053,64,4)
        graph.ndata.update({'ft': ent_feat, 'eh': eh, 'et': et, "ew":ew})
        graph.edata.update({'er': er})
        def edge_attention(edges):
            return {'e': self.leaky_relu(edges.src['ew'] + edges.dst['ew'])}
